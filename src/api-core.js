...
export async function adminSetRole(userId, role) {
  return request(`/api/auth/admin/users/${encodeURIComponent(userId)}/role`, { method: 'PATCH', body: JSON.stringify({ role }) });
}

export async function adminSetActive(userId, isActive) {
  return request(`/api/auth/admin/users/${encodeURIComponent(userId)}/active`, { method: 'PATCH', body: JSON.stringify({ is_active: isActive }) });
}

export async function adminResetPassword(userId, newPassword) {
  return request(`/api/auth/admin/users/${encodeURIComponent(userId)}/reset-password`, { method: 'POST', body: JSON.stringify({ new_password: newPassword }) });
}
