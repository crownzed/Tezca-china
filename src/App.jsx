...
import { AuthControls, AuthModalHost, LeaderboardPanel, ProfilePanel, AdminPanel } from './auth-ui';
...
function AnalyticsPanel({ analytics, onStartRecommended }) {
  ...
  const dataSourceLabel = analytics?.backend_empty ? 'Lịch sử lưu trên máy' : eventCount ? 'Dữ liệu học và lịch ôn' : analytics?.offline ? 'Dữ liệu lưu trên máy' : 'Lịch sử làm bài';
  const latencyLabel = latencyAvg ? (latencyAvg < 1000 ? 'Phản hồi dưới 1 giây' : Math.round(latencyAvg / 1000) + ' giây mỗi câu') : 'Chưa đo tốc độ';
      ...
      <div className="analytics-grid">
        <div className="analytics-main-chart">
          <div className="chart-title-row">
            <div>
              <span className="core-eyebrow">Bản đồ kỹ năng</span>
        ...
        <div className="analytics-side-stack">
          <div className="trend-box">
            <div className="chart-title-row">
              <div>
                <span className="core-eyebrow">Xu hướng</span>
          ...
          <div className="level-box">
            <span className="core-eyebrow">Theo cấp HSK</span>
...
const QUIZ_STRATEGY_MODES = [
  {
    ...
    detail: 'Một dạng bài, phản hồi ngay, chọn mức tự tin và xếp lịch ôn từng câu.',
  },
  {
    id: 'interleaved',
    label: 'Xoay kỹ năng',
    title: 'Trộn kỹ năng nhẹ',
    detail: 'Trộn các kỹ năng liên quan để kiểm tra khả năng liên hệ nghĩa, âm và ngữ cảnh.',
  },
  {
    id: 'repair',
    label: 'Sửa lỗi',
    title: 'Ưu tiên chỗ sai',
    detail: 'Ưu tiên câu dễ lộ lỗi, rồi chèn ngay vòng ôn lại sửa lỗi trong phiên.',
  },
];
      ...
      <main className="core-page page-enter">
        <section className="core-card result-card session-result-card">
          ...
          <ul className="result-narrative">
            <li><ShieldCheck size={16} /> Đã củng cố <strong>{result.correct}</strong> từ qua phần nhớ lại.</li>
          ...
          <div className="session-result-grid">
            <span><ShieldCheck size={18} /><strong>{result.correct}/{result.total}</strong><small>nhớ lại đúng</small></span>
            <span><Wrench size={18} /><strong>{result.repairSuccess}/{result.repairCount}</strong><small>sửa lỗi thành công</small></span>
...
function LearningSession({ plan, fallbackLevel, onExit, onComplete }) {
  ...
  if (completed) {
    return (
      <main className="core-page page-enter">
        <section className="core-card result-card session-result-card">
          <span className="core-eyebrow">Kết quả phiên</span>
          ...
          <div className="session-result-grid">
            ...
            <span><Languages size={18} /><strong>{completed.productionSuccess}/{completed.productionAttempts}</strong><small>câu tự đặt</small></span>
          </div>
          <div className="result-summary">
          ...
          <div className="result-actions">
            ...
            <button className="btn-secondary" onClick={onExit}><BarChart3 size={16} /> Về danh sách hôm nay</button>
          </div>
        </section>
      </main>
    );
  ...
  return (
    <main className="core-page page-enter">
      <section className="core-card core-section-head learning-session-head">
        <div>
          <span className="core-eyebrow">Ôn tập</span>
      ...
      {loading && !item && (
        <section className="core-card empty-state">
          ...
          <p>Hệ thống đang lấy câu hỏi và tạo phiên học.</p>
        </section>
      )}
      ...
      {isIntro && (
        <section className="core-card session-intro-card">
          <span className="core-eyebrow">Hàng chờ hôm nay</span>
      ...
      {isPractice && (
        <section key={item.id} className={`core-card chinese-practice-card chinese-practice-card--${item.type}`} aria-live="polite">
          <div className="question-topline">
            <span>{item.type === 'guided_output' ? 'Đặt câu' : item.type === 'pinyin_typing' ? 'Pinyin' : item.type === 'tone_drill' ? 'Luyện thanh điệu' : item.type === 'micro_reading' ? 'Đọc ngắn' : 'Lớp tiếng Trung'}</span>
          ...
          {item.type === 'character_card' && (
            <div className="character-card-body">
              ...
              <div>
                <span className="core-eyebrow">Họ chữ</span>
                ...
                <div className="metadata-strip">
                  ...
                  <span><strong>{practiceWord.tone_pattern}</strong><small>Thanh điệu</small></span>
                  <span><strong>{practiceWord.topic}</strong><small>Chủ đề</small></span>
                </div>
              </div>
            </div>
          )}
          ...
          {item.type === 'tone_drill' && (
            <div className="practice-stack">
              <span className="core-eyebrow">Mẫu thanh điệu</span>
              <h2>{practiceWord.hanzi}</h2>
              <p>Chọn mẫu thanh điệu đúng cho pinyin: <strong>{practiceWord.pinyin}</strong></p>
              <div className="tone-option-grid">
                {toneOptions(practiceWord.tone_pattern).map(option => (
                  <button key={option} type="button" onClick={() => completePracticeItem({ correct: option === practiceWord.tone_pattern, errorTag: option === practiceWord.tone_pattern ? '' : 'tone_error', feedback: option === practiceWord.tone_pattern ? 'Thanh điệu đúng.' : `Thanh điệu đúng là ${practiceWord.tone_pattern}.` })} disabled={Boolean(practiceFeedback) || submitting}>
                    {option}
                  </button>
                ))}
              </div>
          ...
          {item.type === 'confusion_card' && (
            <div className="practice-stack">
              <span className="core-eyebrow">Cặp dễ nhầm</span>
          ...
          {item.type === 'pinyin_typing' && (
            <div className="practice-stack">
              <span className="core-eyebrow">Gõ pinyin</span>
              <h2>{practiceWord.hanzi}</h2>
          ...
          {item.type === 'micro_reading' && (
            <div className="practice-stack">
              <span className="core-eyebrow">Đọc ngắn</span>
          ...
          {item.type === 'guided_output' && (
            <div className="practice-stack">
              <span className="core-eyebrow">Tự đặt câu</span>
              <h2>{item.prompt}</h2>
              <p>Dùng từ mục tiêu trong một câu ngắn. Điểm đặt câu được tính riêng với điểm nhận biết.</p>
          ...
          {practiceFeedback && (
            <div className={`feedback-panel ${practiceFeedback.correct ? 'feedback-panel--correct' : 'feedback-panel--wrong'}`}>
              <div className="feedback-head">
                ...
                {practiceFeedback.score !== undefined && <span>Đặt câu {practiceFeedback.score}%</span>}
              </div>
    ...
    return (
      <main className="core-page page-enter">
        <section className="core-card result-card general-result-card">
          <span className="core-eyebrow">Kiểm tra tổng quát</span>
  ...
  return (
    <main className="core-page page-enter">
      <section className="core-card core-section-head vocab-head">
        <div>
          <span className="core-eyebrow">Từ vựng</span>
          <h1>Kho từ vựng</h1>
...
function HandwritingPad({ targetWord }) {
  ...
  return (
    <div className="handwriting-panel">
      <div>
        <span className="core-eyebrow">Viết tay</span>
...
function ApplyPractice({ todayPlan, selectedWord, onStartToday }) {
  ...
  return (
    <main className="core-page page-enter">
      <section className="core-card core-section-head">
        <span className="core-eyebrow">Luyện dùng</span>
        <h1>Luyện dùng từ</h1>
        <p>Chuyển từ nhận biết sang tự đặt câu và hội thoại với các từ trọng tâm.</p>
      </section>
      ...
      <section className="apply-grid">
        <div className="core-card apply-panel">
          ...
          {outputResult && <div className={`feedback-panel ${outputResult.correct ? 'feedback-panel--correct' : 'feedback-panel--wrong'}`}><p>{outputResult.feedback}</p><strong>Điểm tự đặt câu {outputResult.score}%</strong></div>}
        </div>

        <div className="core-card chat-panel">
          <div className="chat-head"><span className="core-eyebrow">Trò chuyện</span><strong>{usedTotal}/{targetWords.length} từ trọng tâm</strong></div>
...
function StudyPlan({ todayPlan }) {
  ...
  return (
    <main className="core-page page-enter">
      <section className="core-card core-section-head">
        <span className="core-eyebrow">Kế hoạch</span>
        <h1>Kế hoạch học</h1>
        <p>Đặt thói quen “nếu — thì” và lời nhắc có lý do, không phạt khi lỡ ngày.</p>
      </section>
      ...
      <section className="plan-grid">
        <div className="core-card plan-form">
          ...
          <label className="toggle-row"><input type="checkbox" checked={plan.muted} onChange={event => updatePlan({ muted: event.target.checked })} /><span>Tắt nhắc trên màn hình hôm nay</span></label>
        ...
        <div className="core-card plan-preview">
          <span className="core-eyebrow">Nếu — Thì</span>
...
function Progress({ stats, analytics, onStartRecommended }) {
  ...
  return (
    <main className="core-page page-enter">
      <section className="core-card core-section-head">
        <span className="core-eyebrow">Tiến độ</span>
      ...
      <section className="core-card progress-panel">
        ...
        <div className="progress-copy">
          <span className="core-eyebrow">Gợi ý</span>
      ...
      <section className="core-stats-grid">
        ...
        <StatCard icon={Languages} label="Tự đặt câu" value={`${analytics?.production_readiness || 0}%`} tone="cinnabar" />
      </section>
    </main>
  );
}
...
export default function App() {
  ...
  const navItems = useMemo(() => (isAdmin ? [...NAV, { id: 'admin', label: 'Quản trị', icon: ShieldCheck }] : NAV), [isAdmin]);
  ...
  useEffect(() => {
    if (activeTab === 'admin' && !isAdmin) setActiveTab('dashboard');
  }, [activeTab, isAdmin]);
  ...
  const currentTitle = useMemo(() => {
    ...
    if (activeTab === 'admin') return 'Quản trị người dùng';
  ...
  return (
    <div className="core-app">
      ...
      <section className="core-shell">
        ...
        {!generalCheckLevel && activeTab === 'admin' && isAdmin && <AdminPanel />}
      </section>
      <AuthModalHost />
    </div>
  );
